'''
author Bhanu Prakash (cs15btech11037@iith.ac.in)
       Sahil Yerawar (cs15btech11044@iith.ac.in)
'''

import os, platform, subprocess, re, sys


# change this according to your convienience.
source_dir = './'
benchmark = "perlbench_r"
prog = ""
cmdopt_benchmark = '~/CPUSPEC2017/cpu2017-1_0_1/benchspec/CPU/500.perlbench_r/run/run_base_refrate_mytest-m64.0000/checkspam.pl 2500 5 25 11 150 1 1 1 1'
cmdopt_benchmark_file = ''
# build_dir = '/home/bhanu/llvm-build/build/'
build_dir = '~/LLVM/llvm_build'
lib_timer = '/lib/LLVMTimer.so'
opt_basic_flags = '-mem2reg -loop-simplify'
opt_timer_flags = '-count-loops -insertstr -inserttimer -print-func-name'
ll_generate_flags = '-S -emit-llvm '
# clang = 'clang++-5.0'
clang = 'clang++'
opt = 'opt'
llvmdis = 'llvm-dis'
numiter=65	#for llvm timer
#you may want to add some more flags for timer
opt_flags = ["-globalopt -demanded-bits -branch-prob -inferattrs -ipsccp -dse -loop-simplify -scoped-noalias -barrier -adce -deadargelim -memdep -licm -globals-aa -rpo-functionattrs -basiccg -loop-idiom -forceattrs -mem2reg -simplifycfg -early-cse -instcombine -sccp -loop-unswitch -loop-vectorize","-loop-unswitch","-mem2reg  -simplifycfg  -loops  -lcssa -loop-simplify -loop-rotate -loop-unroll -unroll-count=3 -unroll-allow-partial ","-loop-sink","-inline","-loop-reroll","-loop-reduce","-loop-load-elim","-licm","-loop-instsimplify","-loop-distribute"]
remove_llbcfiles = True

#use this or manually write the clock speed
def get_processor_name():
    if platform.system() == "Windows":
        return platform.processor()
    elif platform.system() == "Darwin":
        os.environ['PATH'] = os.environ['PATH'] + os.pathsep + '/usr/sbin'
        command ="sysctl -n machdep.cpu.brand_string"
        return subprocess.check_output(command).strip()
    elif platform.system() == "Linux":
        command = "cat /proc/cpuinfo"
        all_info = subprocess.check_output(command, shell=True).strip()
        for line in all_info.split("\n"):
            if "model name" in line:
                return re.sub( ".*model name.*:", "", line,1)
    return ""


def gettimes(file):
	global numiter
	with open(file,"r") as f:
		#you may want to remove some starting lines, which is non-timer output (depending on the benchmark)
		#2 (header) + 6*numiter (iterations)+ 2 (header) + tuples start here
		wait = 4+6*numiter+2
		i=0
		times = []
		total = 0.0
		#processor clock speed (works for linux)
		processor = get_processor_name()
		x = [word.strip() for word in processor.split(' ')][-1][:-3]
		print(x)
		clock = float(x)*1e9	#assuming it is GHz
		frontier_flag = False # Sahil: This is the flag which tells whether to ignore or to analyze the line
		for line in f:
			'''
			I have modified the LLVMTimer  such that it insert the line "Profiler output"
			before the timer analysis, we check whether we get this line, if yes, flip
			the boolean flag. Otherwise, until we reach there, we ignore the line.
			'''
			if "Profiler Output" in line:
                		frontier_flag = True
                		continue
            		if not frontier_flag or line == "\n":
                		continue
            		else:
			# if i<wait:
			# 	i+=1
			# else:
				x = [float(word.strip()) for word in line.split(',')]
				print(x)
				times.append(x)
				total+=times[-1][2]
	total /= clock
	return total



def createExec():
	global source_dir
	global benchmark
	global opt_flags
	global opt_basic_flags
	global opt_timer_flags
	global opt
	global clang
	global llvmdis
	global remove_llbcfiles
	global build_dir
	global lib_timer
	new_dir = source_dir+"temp/"
	os.system("mkdir "+new_dir)
	os.system(opt+" "+opt_basic_flags+" "+source_dir+benchmark+".ll -o "+new_dir+benchmark+"_base.bc")
	#below call maynot be required, if so use _base.bc for further steps
	os.system(llvmdis+" "+new_dir+benchmark+"_base.bc -o "+new_dir+benchmark+"_base.ll")
	os.system("opt -load "+build_dir+lib_timer+" "+opt_timer_flags+" "+new_dir+benchmark+"_base.ll"+" -o "+new_dir+benchmark+"_orig.bc")
	os.system(clang+" "+new_dir+benchmark+"_orig.bc -o "+new_dir+benchmark+"_orig.out")
	for i in range(len(opt_flags)):
		os.system("opt -load "+build_dir+lib_timer+" "+opt_flags[i]+" "+opt_timer_flags+" "+new_dir+benchmark+"_base.ll"+" -o "+new_dir+benchmark+"_"+str(i+1)+".bc")
		os.system(clang+" "+new_dir+benchmark+"_"+str(i+1)+".bc -o "+new_dir+benchmark+"_"+str(i+1)+".out")

	if remove_llbcfiles:
		os.system("rm "+new_dir+"*.bc")
		os.system("rm "+new_dir+benchmark+"_base.ll")


def execAll():
	global source_dir
	global benchmark
	global opt_flags
	global cmdopt_benchmark
	new_dir = source_dir+"temp/"
	#below line needs some modification (executing the binary)
	os.system(new_dir+benchmark+"_orig.out "+cmdopt_benchmark+" > "+new_dir+benchmark+"_orig.txt")
	#assuming the llvm timer output is in new_dir/benchmark_orig.txt
	def_time = gettimes(new_dir+benchmark+"_orig.txt")
	times = []
	for i in range(len(opt_flags)):
		#below line needs some modification (executing the binary)
		os.system(new_dir+benchmark+"_"+str(i+1)+".out "+cmdopt_benchmark+" > "+new_dir+benchmark+"_"+str(i+1)+".txt")
		#assuming the llvm timer output is in new_dir/benchmark_<optnum>.txt
		times.append((gettimes(new_dir+benchmark+"_"+str(i+1)+".txt"),opt_flags[i]))

    # print(times)
	times.sort(key=lambda x: x[0])
	print "\n\n****Flag Mining - Result****\n"
	print "default time = "+str(def_time*1000)+" ms"
    	print "\nGood Flags:- \n"
    	for i in range(len(times)):
		if times[i][0] <= def_time:
			print times[i][1]+" - "+str(times[i][0]*1000)+" ms"

	#printing the top 5 flags
	print "\n"
	for i in range(5):
		print times[i][1]+" - "+str(times[i][0]*1000)+" ms"




def createExec1():
	global source_dir
	global benchmark
	global opt_flags
	global opt_basic_flags
	global opt_timer_flags
	global opt
	global clang
    	global prog
	global llvmdis
	global remove_llbcfiles
	global build_dir
	global lib_timer
	new_dir = source_dir+"temp/"
    	prog_name = prog.split(".")[0]
	os.system("mkdir "+new_dir)
    	os.system(clang+" "+ll_generate_flags+" "+source_dir+prog+" -o "+source_dir+prog_name+".ll")
	os.system(opt+" "+opt_basic_flags+" "+source_dir+prog_name+".ll -o "+new_dir+prog_name+"_base.bc")
	#below call maynot be required, if so use _base.bc for further steps
	os.system(llvmdis+" "+new_dir+prog_name+"_base.bc -o "+new_dir+prog_name+"_base.ll")
	os.system("opt -load "+build_dir+lib_timer+" "+opt_timer_flags+" "+new_dir+prog_name+"_base.ll"+" -o "+new_dir+prog_name+"_orig.bc")
	os.system(clang+" "+new_dir+prog_name+"_orig.bc -o "+new_dir+prog_name+"_orig.out")
	for i in range(len(opt_flags)):
		os.system("opt -load "+build_dir+lib_timer+" "+opt_flags[i]+" "+opt_timer_flags+" "+new_dir+prog_name+"_base.ll"+" -o "+new_dir+prog_name+"_"+str(i+1)+".bc")
		os.system(clang+" "+new_dir+prog_name+"_"+str(i+1)+".bc -o "+new_dir+prog_name+"_"+str(i+1)+".out")

	if remove_llbcfiles:
		os.system("rm "+new_dir+"*.bc")
		os.system("rm "+new_dir+prog_name+"_base.ll")


def execAll1():
	global source_dir
	global benchmark
	global opt_flags
	global cmdopt_benchmark_file
    	global prog
    	prog_name = prog.split(".")[0]
	new_dir = source_dir+"temp/"
	#below line needs some modification (executing the binary)
	os.system(new_dir+prog_name+"_orig.out "+cmdopt_benchmark_file+" > "+new_dir+prog_name+"_orig.txt")
	#assuming the llvm timer output is in new_dir/benchmark_orig.txt
	def_time = gettimes(new_dir+prog_name+"_orig.txt")
	times = []
	for i in range(len(opt_flags)):
		#below line needs some modification (executing the binary)
		os.system(new_dir+prog_name+"_"+str(i+1)+".out "+cmdopt_benchmark_file+" > "+new_dir+prog_name+"_"+str(i+1)+".txt")
		#assuming the llvm timer output is in new_dir/benchmark_<optnum>.txt
		times.append((gettimes(new_dir+prog_name+"_"+str(i+1)+".txt"),opt_flags[i]))

    # print(times)
	times.sort(key=lambda x: x[0])
	print "\n\n****Flag Mining - Result****\n"
	print "default time = "+str(def_time*1000)+" ms"
    	print "\nGood Flags:- \n"
    	for i in range(len(times)):
		if times[i][0] <= def_time:
			print times[i][1]+" - "+str(times[i][0]*1000)+" ms"

	#printing the top 5 flags
	print "\n"
	for i in range(5):
		print times[i][1]+" - "+str(times[i][0]*1000)+" ms"



if len(sys.argv)==1:
	print "assuming the benchmark namd_r.ll"
    	createExec()
    	execAll()
else:
	if len(sys.argv)==2:
		benchmark = sys.argv[1]
		createExec()
        	execAll()
    	elif len(sys.argv) ==3:
        	if "-file" in sys.argv[1]:
            		prog = sys.argv[2]
            		createExec1()
            		execAll1()
	else:
		print "too many parameters ..exiting"
		sys.exit()
